...
class LabelModelService:
    ...
    def label_model(self, run_object, selectedModelLabel, n_epochs=100, log_freq=10, seed=123,
        ...
        if selectedModelLabel == 'Majority Vote':
            ...
            run_object.save()
            return self.__majority_vote_label(run_object, numbers_of_labels)
        elif selectedModelLabel == 'Train Label Model':
            label = LabelSummary.objects.get_or_create(type='P')
            run_object.labelmodel = label[0]
            run_object.save()
            return self.__train_label_model(base_learning_rate, l2, log_freq, n_epochs, run_object, seed, numbers_of_labels)
        return {"message": "Choose a valid label service"}

    def __train_label_model(self, base_learning_rate, l2, log_freq, n_epochs, run_object, seed, numbers_of_labels):
    ...
    def __majority_vote_label(self, run_object, numbers_of_labels):
