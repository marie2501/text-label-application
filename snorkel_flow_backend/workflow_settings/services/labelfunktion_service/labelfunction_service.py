...
class LabelfunctionService:
    ...
    def __test_on_dataset(self, code, file_path, imports, name, workflow_id, labels_code=''):
        try:
            L_train, L_unlabeled, dataframe_train, dataframe_unlabeled, text_list_train_gold_labels, lfs = self.computeLabels(code, file_path, imports,
                                                                                            labels_code, name)

            df_combined = self.__get_datapoints_with_labels(L_train, L_unlabeled, dataframe_train, dataframe_unlabeled)
            ...
            return status.HTTP_200_OK, {'summary': labelsummary_unlabeled, 'summary_train': labelsummary_train, 'df_combined': df_combined}
        except:
            data = str(sys.exc_info())
            return status.HTTP_400_BAD_REQUEST, data

    def __get_datapoints_with_labels(self, L_train, L_unlabeled, dataframe_train, dataframe_unlabeled):

        dataframe_train['computed_Labels'] = L_train.flatten().tolist()

        dataframe_unlabeled['computed_Labels'] = L_unlabeled.flatten().tolist()

        dataframe_unlabeled = dataframe_unlabeled[['entity_id', 'corpus_id', 'text', 'splitting_id', 'computed_Labels']]
        dataframe_train = dataframe_train[['entity_id', 'corpus_id', 'text', 'splitting_id', 'computed_Labels', 'CLASS']]

        df_combined = pd.concat([dataframe_train, dataframe_unlabeled], ignore_index=True)
    ...
    def computeLabels(self, code, file_path, imports, labels_code, name):
        import_code = imports[0].code
        exec(import_code, locals())
        ...
        return L_train, L_unlabeled, dataframe_train, dataframe_unlabeled, text_list_train_gold_labels, lfs
